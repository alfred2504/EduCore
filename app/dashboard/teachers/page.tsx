...
export default async function TeachersPage() {
  ...
  const roleOnlyUsers = await prisma.user.findMany({ where: { role: 'TEACHER', teacher: { is: null } }, orderBy: { createdAt: 'desc' } });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href="/dashboard/teachers/new" className="rounded-xl bg-blue-600 px-4 py-2 text-white">
          Add Teacher
        </Link>
      </div>
          ...
          <tbody>
              ...
              <tr key={teacher.id} className="border-b">
                ...
                <td className="px-6 py-4">
                  <div className="flex gap-3">
                    <Link href={`/dashboard/teachers/${teacher.id}`} className="text-blue-600">View</Link>
                    <Link href={`/dashboard/teachers/edit/${teacher.id}`} className="text-yellow-600">Edit</Link>
                  </div>
                </td>
              </tr>
            ))}
            ...
            {roleOnlyUsers.map((u) => (
              <tr key={u.id} className="border-b">
                <td className="px-6 py-4">{u.name} <span className="text-sm text-slate-400">(user only)</span></td>
